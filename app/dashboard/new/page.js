    ...
    const handleGenerate = async () => {
        ...
        try {
            const response = await fetch('/api/generate', {
                ...
                body: JSON.stringify({ category, contentPrompt, colorPrompt, functionalityPrompt }),
            });
                ...
                <div className="bg-zinc-900/80 rounded-xl border border-zinc-800 flex flex-col h-full min-h-[600px] overflow-hidden backdrop-blur-sm">
                    {!result ? (
                        ...
                        <>
                            <div className="flex border-b border-zinc-700 bg-zinc-950">
                                {['html', 'css', 'js'].map((type) => (
                                    <button
                                        ...
                                        className={`flex-1 py-3 text-sm font-medium transition-colors ${activeTab === type
                            ...
                            <div className="flex-1 relative bg-[#0d1117] p-4 font-mono text-sm overflow-auto">
                            ...
                            <div className="p-4 border-t border-zinc-700 bg-zinc-900 flex justify-end space-x-4">
                                <button
                                    onClick={() => {
                                        ...
                                        if (result.html) downloadFile(result.html, 'index.html');
                                        if (result.css) downloadFile(result.css, 'style.css');
                                        if (result.js) downloadFile(result.js, 'script.js');
                                    }}
                                    className="text-sm text-zinc-400 hover:text-white transition-colors"
                                >
                                    Download All Files
                                </button>
                                <button className="px-4 py-2 bg-purple-600 rounded text-sm font-medium hover:bg-purple-700 transition-colors">Open Preview</button>
                            </div>
                        </>
                    )}
                </div>
